...
if (managerCount === 0) {
  const insert = db.prepare(
    'INSERT INTO managers (name, title, sort, active, created_at) VALUES (?, ?, ?, 1, ?)'
  );
  transaction(() => {
    DEFAULT_MANAGERS.forEach((name, idx) => insert.run(name, '', idx + 1, nowIso()));
  });
}
